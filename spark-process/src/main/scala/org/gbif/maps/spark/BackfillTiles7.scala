...
object BackfillTiles7 {
  ...
  def build(sc :SparkContext, df : DataFrame): Unit = {
    ...
    }).reduceByKey(_ + _, 500).map(r => { // 27mins ,40sec in total into HBase(!) - grouping to the features(!)
    ...
    val appendVal = (m: DensityTile, v: (Int,Int,Int,Feature.BasisOfRecord,Int)) => {m.collect((v._1, v._2), v._4, v._3, v._5)}
    val merge = (m1: DensityTile, m2: DensityTile) => {m1.merge(m2)}
    var tiles2 = tiles.aggregateByKey(new DensityTile)(appendVal, merge)
    ...
    (MIN_ZOOM to MAX_ZOOM).reverse.foreach(z => {
      ...
      if (z != MAX_ZOOM) {
        tiles2 = tiles2.map(t => {
          ...
          val x = zxy(1).toInt
          val y = zxy(2).toInt
          val newTile = t._2.downscale(zoom, x, y)
      ...
      tiles2.mapValues(tile => {
        ...
        tile.build().foreach(f => {
          val point = GEOMETRY_FACTORY.createPoint(new Coordinate(f._1._1.toDouble, f._1._2.toDouble));
