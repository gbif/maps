...
object AppPoints4 {
  ...
  // TODO: how do we run this in the cluster properly?
  def main(args: Array[String]) {
    ...
    // Maps with more features than this will be pushed into a tile pyramid in addition to the first 100000
    ...
    val initialCounts = df.flatMap(row => {
      ...
      val speciesKey = row.getInt(row.fieldIndex("specieskey")) // TODO: taxonKey stuff (!)
    ...
    }).repartition(100).sortByKey(true).map(r => {
    ...
    val tilePyramid = initialCounts
      .filter(r => {
        b_largeVals.value.contains((r._1._1, r._1._2))
      }).flatMap(r => {
        val mapType = r._1._1
        val mapKey = r._1._2
        val lat = r._1._3
      ...
      mutable.Map[(Int, Int),mutable.Map[Int, Int]]())((agg, v) => {
          ...
          // merge and convert into a mutable object (required to allow repeated calling)
        ...
        r._2.foreach(px => {
          ...
          val meta = new java.util.HashMap[String, Any]() // TODO: add years to the VT
    ...
    pointData.saveAsNewAPIHadoopFile("hdfs://c1n1.gbif.org:8020/tmp/zp4", classOf[ImmutableBytesWritable], classOf[KeyValue],
        classOf[HFileOutputFormat], outputConf)

    tilePyramid.saveAsNewAPIHadoopFile("hdfs://c1n1.gbif.org:8020/tmp/zt4", classOf[ImmutableBytesWritable], classOf[KeyValue],
        classOf[HFileOutputFormat], outputConf)
