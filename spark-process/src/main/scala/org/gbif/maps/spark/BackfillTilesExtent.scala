...
/**
  * THIS TRIAL IS A FAILURE - you cannot merge up like this!!!
  * A builder of HFiles for a tile pyramid.
  */
object BackfillTilesExtent {
  ...
  def build(sc :SparkContext, df : DataFrame, keys: Set[String], config: MapConfiguration, projectionConfig: ProjectionConfig) = {
    ...
    var tiles = df.flatMap(row => {
      ...
      // skip completely if we can't plot it
      if (projection.isPlottable(lat, lng)) {
        ...
        // only process when it falls within those we are instructed to
        if (mapKeys.size>0) {

          val bufferSize = 8 // TODO: move to config
          ...
          // identify the tiles that this global pixel falls on, considering the buffer extent means it can fall
          // on multiple tiles
          val targetTiles = mutable.ArrayBuffer[(Long, Long)]() // XY addressing
          targetTiles += ((x,y)) // primary tile

          // if we are buffering, locate the tile coordinates for adjacent tiles
          if (bufferSize > 0 ) {
            // locate east and west tile x coordinate, handling the dateline (infinite wrap)
            val westX = if (x==0) ((2<<z-1)-1) else x-1
            val eastX = if (x==((2<<z-1)-1)) 0 else x+1

            // tiles to the east and west
            targetTiles += ((westX,y)) // W
            if (eastX != westX) {
              targetTiles += ((eastX,y)) // E
            }

            // tiles to the north
            if (y>0) {
              targetTiles += ((westX,y-1)) // NW
              targetTiles += ((x,y-1))     // N
              if (eastX != westX) {
                targetTiles += ((eastX,y-1)) // NE
              }
            }

            // tiles to the south (unless on bottom row)
            if (y<((2<<z-1)-1)) {
              targetTiles += ((westX,y+1)) // SW
              targetTiles += ((x,y+1))     // S
              if (eastX != westX) {
                targetTiles += ((eastX,y+1)) // SE
              }
            }
          }

          // for each of the maps that this record affects
          mapKeys.foreach(mapKey => {

            // for each of the tiles we've identified this may fall on
            targetTiles.foreach(tileXY => {

              val tileLocalXY = Tiles.toTileLocalXY(globalXY, tileXY._1, tileXY._2,
                projectionConfig.tileSize) // pixel on the tile of interest
              val px = tileLocalXY.getX.asInstanceOf[Short] // note: rounds here
              val py = tileLocalXY.getY.asInstanceOf[Short] // note: rounds here

              // only emit if the pixel is within the buffered area
              if (
                px > -bufferSize && px < projectionConfig.tileSize + bufferSize &&
                py > -bufferSize && py < projectionConfig.tileSize + bufferSize
              ) {
                val pixel = MapUtils.encodePixel(px, py)
                val zxy = MapUtils.toZXY(z,tileXY._1,tileXY._2) // the encoded tile address
    ...
    }).partitionBy(new TileGroupPartitioner(200)) // TODO: configurify like the point one!
