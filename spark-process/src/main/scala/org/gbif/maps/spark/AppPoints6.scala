...
object AppPoints6 {
  ...
  def main(args: Array[String]) {
    ...
    initialCounts.repartition(200)

    // causes issue?
    //initialCounts.persist(StorageLevel.MEMORY_AND_DISK_SER)
    ...
    val pointData = initialCounts.map(r => {
      ...
      var runningCount = 0
      val iter = r.iterator
      while (iter.hasNext && runningCount < POINT_THRESHOLD) {
        val f = iter.next()
    ...
    pointData.saveAsNewAPIHadoopFile("hdfs://c1n1.gbif.org:8020/tmp/zp16", classOf[ImmutableBytesWritable],
        classOf[KeyValue], classOf[HFileOutputFormat], outputConf)

    tilePyramid.saveAsNewAPIHadoopFile("hdfs://c1n1.gbif.org:8020/tmp/zt14", classOf[ImmutableBytesWritable],
        classOf[KeyValue], classOf[HFileOutputFormat], outputConf)
