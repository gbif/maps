...
object AppPoints8 {
  ...
  def main(args: Array[String]) {
    ...
    val df = sqlContext.read.parquet("/Users/tim/dev/data/map.parquet").repartition(10)
    ...
    val p1 = initialCounts.map(r => {
      ((r._1._1 + ":" + r._1._2), (r._1._3, r._1._4, r._1._5, r._1._6, r._2))
    })

    val res = MLPairRDDFunctions.fromPairRDD(p1).topByKey(POINT_THRESHOLD).mapValues(r => {
    ...
    println("points: " + res.count())


    /*
    val p = initialCounts.map(r => {
      ((r._1._1 + ":" + r._1._2), (r._1._3, r._1._4, r._1._5, r._1._6, r._2))
    }).sortByKey(true).mapPartitions(iter => {
      var runningCount = 0
      var lastKey = ""
      var data = mutable.ArrayBuffer[((String),(Double, Double, Int, String, Int))]()
      while (iter.hasNext) {
        val f = iter.next()
        if (f._1 != lastKey) {
          runningCount = 0
        ...
        if (runningCount < POINT_THRESHOLD) data += f
      }
      data.iterator
    }).groupByKey().map(r => {
      val builder = PointFeature.PointFeatures.newBuilder();
    ...
    */
    ...
    val tilePyramid = initialCounts.filter(r => {b_largeVals.value.contains((r._1._1, r._1._2))}).flatMap(r => {
        ...
        (MIN_ZOOM to MAX_ZOOM).flatMap(z => {
          // TODO: BoR
    ...
    */
    ...
    // share the lookup among the cluster to allow distributed filtering
    val b_largeVals = sc.broadcast(largeVals)
    val tileData = initialCounts.filter(r => {b_largeVals.value.contains((r._1._1, r._1._2))})

    val z14 = tileData.map(r => {
      val mapType = r._1._1
      val mapKey = r._1._2
      val lat = r._1._3
      val lng = r._1._4
    ...
    z14.map(r => {
      ...
      (k, row)
    }).saveAsNewAPIHadoopFile("hdfs://c1n1.gbif.org:8020/tmp/ml_zt2_z14", classOf[ImmutableBytesWritable],
      classOf[KeyValue], classOf[HFileOutputFormat], outputConf)

    val z13 = z14.map(r => {
      //(r._1._1, r._1._2, r._1._3, r._1._4, r._1._5)
      // (type,key,zoom,x,y)
      ((r._1._1, r._1._2, r._1._3<<2, r._1._4<<2, r._1._5<<2), r._2)
    }).aggregateByKey(
      mutable.Map[(Int, Int), mutable.Map[Int, Int]]())((agg1, agg2) => {
      mutable.Map() ++ Maps.merge(agg1, agg2)
    }, (agg1, agg2) => {
      mutable.Map() ++ Maps.merge(agg1, agg2)
    })
      ...
      .saveAsNewAPIHadoopFile("hdfs://c1n1.gbif.org:8020/tmp/ml_zt_z13", classOf[ImmutableBytesWritable],
      classOf[KeyValue], classOf[HFileOutputFormat], outputConf)
    ...
    res.saveAsNewAPIHadoopFile("hdfs://c1n1.gbif.org:8020/tmp/ml_zp5", classOf[ImmutableBytesWritable],
        classOf[KeyValue], classOf[HFileOutputFormat], outputConf)


    tilePyramid.saveAsNewAPIHadoopFile("hdfs://c1n1.gbif.org:8020/tmp/ml_zt_z8", classOf[ImmutableBytesWritable],
        classOf[KeyValue], classOf[HFileOutputFormat], outputConf)
        */
