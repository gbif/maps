...
object BackfillTilesBuffer2 {
  ...
  def build(sc :SparkContext, df : DataFrame, keys: Set[String], config: MapConfiguration, projectionConfig: ProjectionConfig) = {
    ...
    (projectionConfig.minZoom to projectionConfig.maxZoom).reverse.foreach(z => {
        ...
        tiles3 = tiles3.map(t => {
          ...
          t._2.foreach(keyVal => {
            ...
            keyVal._2.foreach(feature => {
              ...
              // Important(!) We only emit pixels that fall on the current tile and exclude any that are in the buffer
              // If an e.g. eastzone buffer pixel of x=260 on a 256px tile  goes through the following code it would be
              // painted at 130px incorrectly.  Buffers are recreated if needed after the tiles are downscaled.
              if (pixel._1 >= 0 && pixel._1 < projectionConfig.tileSize &&
                  pixel._2 >= 0 && pixel._2 < projectionConfig.tileSize) {
                // the pixel is within the boundary of the tile, excluding buffer
                val year = decoded._2
      ...
      val bufferSize = 8 // TODO - move into config!

      // extract buffer pixels
      val tiles4 = tiles3.flatMap(t => {
        ...
        // TODO: handle datelines and proper yZ>0 etc
        ...
        // TODO: NE,NW,SW,SE because I need to handle date line
        // TODO: Those x and y values tests are BS above
