...
object BackfillTiles5 {
  ...
  def build(sc :SparkContext, df : DataFrame): Unit = {
    ...
    }).map(r => { // 18 minutes into HBase(!) - points only
    ...
    // After adding stuff to encode tiles (only writing the pixels) we see:
    // - failed with first attempt
    // 41(ish) mins with a 1000 repartition in there...
