  ...
  def build(sc :SparkContext, df : DataFrame): Unit = {
    ...
    var tiles = df.flatMap(row => {
      ...
      if (lat >= -85 && lat <= 85) {
        ...
        val bor = BASIS_OF_RECORD(row.getString(row.fieldIndex("basisofrecord")))
        val year = if (row.isNullAt(row.fieldIndex("year"))) null.asInstanceOf[Short]
        else row.getInt((row.fieldIndex("year"))).asInstanceOf[Short] // TODO!!!
        ...
        // Records always contribute to the "ALL" map
        val mapKey = toMapKey(MAPS_TYPES("ALL"), 0)
        val zxy = toZXY(z,x,y)
        ...
        if (mapViewsToTile.value.contains(datasetKey)) {
          res += (((datasetKey, zxy, pixel, bor, year),1))
        }
        ...
        if (mapViewsToTile.value.contains(publisherKey)) {
          res += (((publisherKey, zxy, pixel, bor, year),1))
        }
        ...
        if (!row.isNullAt(row.fieldIndex("countrycode"))) {
          ...
          if (mapViewsToTile.value.contains(countryCode)) {
            res += (((countryCode, zxy, pixel, bor, year),1))
          }
        }
        ...
        taxonIDs.foreach(id => {
          ...
          if (mapViewsToTile.value.contains(taxonKey)) {
            res += (((taxonKey, zxy, pixel, bor, year),1))
          }
        })
    ...
    val merge = (m1: MMap[Long,Int], m2: MMap[Long,Int]) => {
      ...
      */
      ...
      m1 ++= m2 // TODO: Can we reuse here or not?
    }
