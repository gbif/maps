...
DROP TABLE IF EXISTS tim.occurrence_wgs84;
CREATE TABLE tim.occurrence_wgs84 STORED AS parquet AS
SELECT
 xy.x AS x, xy.y AS y,
...
FROM (
  SELECT
    project(decimalLatitude, decimalLongitude, 'EPSG:4326', 16) AS xy,
    datasetKey, publishingOrgKey, publishingCountry, networkKey,
    countryCode, basisOfRecord, kingdomKey, phylumKey, classKey, orderKey, familyKey,
...
sudo rclone sync c5:/user/hive/warehouse/tim.db/occurrence_wgs84 /var/lib/clickhouse/user_files/occurrence_wgs84/.
```
...
SELECT toUInt32(x), toUInt32(y), datasetkey, publishingorgkey,
  ...
  year, occcount
FROM file('occurrence_wgs84/*', Parquet);
```
