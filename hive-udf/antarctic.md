...
DROP TABLE IF EXISTS tim.occurrence_antarctic;
CREATE TABLE tim.occurrence_antarctic STORED AS parquet AS
SELECT
 xy.x AS x, xy.y AS y,
...
sudo rclone sync c5:/user/hive/warehouse/tim.db/occurrence_antarctic /var/lib/clickhouse/user_files/occurrence_antarctic/.
```
...
SELECT toUInt32(x), toUInt32(y), datasetkey, publishingorgkey,
  ...
  year, occcount
FROM file('occurrence_antarctic/*', Parquet);
```
