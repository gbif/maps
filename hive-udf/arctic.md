...
DROP TABLE IF EXISTS tim.occurrence_arctic;
CREATE TABLE tim.occurrence_arctic STORED AS parquet AS
SELECT
 xy.x AS x, xy.y AS y,
...
sudo rclone sync c5:/user/hive/warehouse/tim.db/occurrence_arctic /var/lib/clickhouse/user_files/occurrence_arctic/.
```
...
SELECT toUInt32(x), toUInt32(y), datasetkey, publishingorgkey,
  ...
  year, occcount
FROM file('occurrence_arctic/*', Parquet);
```
