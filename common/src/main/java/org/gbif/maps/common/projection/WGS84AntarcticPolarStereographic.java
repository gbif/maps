...
class WGS84AntarcticPolarStereographic extends WGS84Azimuthal {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(WGS84AntarcticPolarStereographic.class);
  ...
  public Double2D[] tileBoundary(int z, long x, long y, double tileBuffer) {
    ...
    // A(x1,y1) B(x2,y1)
    // C(x1,y2) D(x2,y2)
    final Double2D pA = fromGlobalPixelXY(x1b, y1b, z);
    final Double2D pB = fromGlobalPixelXY(x2b, y1b, z);
    final Double2D pC = fromGlobalPixelXY(x1b, y2b, z);
    final Double2D pD = fromGlobalPixelXY(x2b, y2b, z);
    ...
    if ((x < seamCoordinate && y < seamCoordinate) || (x >= seamCoordinate && y >= seamCoordinate)) {
      ...
      if (tileBuffer > 0 && !pole) {
        ...
        if (x+1 == seamCoordinate) { // right of top left quadrant
          ...
          Double2D pForLngAM = pA;
          // Over the antimeridian
          pForLng1 = new Double2D(pForLngAM.getX(), pForLngAM.getY()+360);
        }
      }
    } else {
      ...
      if (pole) {
        pForLng1 = fromGlobalPixelXY(x1u, y1u, z); // A (unbuffered)
        pForLng2 = fromGlobalPixelXY(x2u, y2u, z); // D (unbuffered)
      } else {
        pForLng1 = pA;
        pForLng2 = pD;
      }
